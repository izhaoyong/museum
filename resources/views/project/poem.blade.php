...
@if ($search)
	<div class="container">
		<p>数据：<strong>{{$search."\t"}}</strong> 搜索到{{ count($poems)}}条结果:</p>
	</div>
	@endif
			...
			<section class="grid" id="container">
				...
				@foreach ($poems as $poem)
					<a class="grid__item" href="#">
						<h2 class="title title--preview">{{ $poem->title }}</h2>
						<div class="loader"></div>
						<span class="category">{{ $poem-> author }}</span>
						<div class="meta meta--preview">
							<img class="lazy meta__avatar" src="{{ asset('/assets/img/logo.png') }}" data-original="{{ asset('content/poem/image')."/".$poem->picture.".jpg" }}" alt="图片暂缺" onerror="imgError(this);">
						</div>
					</a>
				@endforeach
			</section>
				...
				@foreach ($poems as $poem)
					<article class="content__item">
						...
						<div class="meta meta--full">
							...
							<img class="meta__avatar" data-src="{{ asset('content/poem/image')."/".$poem->picture.".jpg" }}" alt="图片暂缺" >

							<span class="meta__subTitle">{{ $poem->title }}</span>
						...
						<div class="picture">
							<img class="pic_photo" data-src="{{ asset('content/poem/image')."/".$poem->picture.".jpg" }}" alt="图片暂缺" />
						</div>

						<div class="detail" style="max-height: calc(85vh - 60px); overflow:auto">
							<h2 class="title title--full">{{ $poem-> author }}</h2>
							<p>{{ $poem-> content }}</p>
							<p>{{ $poem-> comment }}</p>
						</div>
					</article>
