...
@if ($search)
	<div class="container">
		<p>数据：<strong>{{$search."\t"}}</strong> 搜索到{{ count($oldbeijings)}}条结果:</p>
	</div>
	@endif
	...
	<div class="wrapper" id="panel" style="width:100%">
		<div id="theSidebar" class="sidebar">
			...
			<div class="info">
				<h3>话说老北京主要是对北京语言文化图解词典编创。本项目以《话说老北京》这部京味图画书为蓝本，整理成为一部能听能看的老北京故事图鉴，通过图鉴记录过去的人玩什么、吃什么、穿什么，街道上卖什么，流行什么。</h3>
			</div>

			<button class="detailButton" onclick="location.href='/oldbeijing.html';">详细介绍>></button>
		</div>
			...
			<section class="grid" id="container">
				...
				@foreach ($oldbeijings as $oldbeijing)
					<a class="grid__item" href="#">
						<h2 class="title title--preview">{{ $oldbeijing->entry }}</h2>
						<div class="loader"></div>
						<span class="category"></span>
						<div class="meta meta--preview">
							<img class="lazy meta__avatar" src="{{ asset('/assets/img/logo.png') }}" data-original="{{ asset('content/oldbeijing/image')."/".$oldbeijing->item_id }}.jpg" alt="图片暂缺" onerror="imgError(this);">
						</div>
					</a>
				@endforeach
			</section>
				...
				@foreach ($oldbeijings as $oldbeijing)
					<article class="content__item">
						...
						<div class="meta meta--full">
							...
							<img class="meta__avatar" data-src="{{ asset('content/oldbeijing/image')."/".$oldbeijing->item_id }}.jpg" alt="图片暂缺" onerror="imgError(this);">

							<span class="meta__subTitle">{{ $oldbeijing->entry }}</span>

							<div class="meta__misc--seperator">
								北京话 ：{{ $oldbeijing-> beijing_entry }}</br>
								发音 ：{{ $oldbeijing-> pronunciation }}</br>
								释义 ：{{ $oldbeijing->interpretation }}</br>
								声音 :
								<button type="button" class="btn btn-default" onclick="playSound(this);">
							          	<audio src="{{asset('content/oldbeijing/sound')."/".$oldbeijing->item_id}}.wav">
							          	</audio>
						...
						<div class="picture">
							<img class="pic_photo" data-src="{{ asset('content/oldbeijing/image')."/".$oldbeijing->item_id }}.jpg" alt="图片暂缺" onerror="imgError(this);"/>
						</div>
