		...
		@foreach ($chants as $chant)
			<a class="grid__item" href="#">
				...
				<div class="meta meta--preview">
					<img class="meta__avatar lazy" src="{{ asset('/assets/img/logo.png') }}" data-original="{{ asset('content/chant')."/".$chant->category.'/'.$filename.".jpg" }}"  onerror="imgError(this);">
				</div>
			</a>
		@endforeach
	</section>
			...
			@foreach ($chants as $chant)
				<article class="content__item">
					<div class="meta meta--full">
						...
						<img class="meta__avatar" alt="" data-src="{{ asset('content/chant')."/".$chant->category.'/'.$filename.".jpg" }}" alt="图片暂缺" onerror="imgError(this);">
					...
					<div class="picture">
						...
						<video class="video" style="" controls="controls" preload="none">
								<?php
									$name = str_replace('flv', 'mp4', $chant->media);
								?>
								<source src="{{ asset('content/chant')."/".$chant->category.'/'.$name }}"  />
								<?php
									$name = str_replace('flv', 'ogg', $chant->media);
								?>
								<source src="{{ asset('content/chant')."/".$chant->category.'/'.$name }}"  />
								<?php
									$name = str_replace('flv', 'webm', $chant->media);
								?>
								<source src="{{ asset('content/chant')."/".$chant->category.'/'.$name }}"  />
						</video>						
